...
const EditEmployee = () => {
  ...
  useEffect(() => {
    axios
      .get("backen-production-e1ec.up.railway.app/api/units")
      .then((res) => setUnits(res.data))
      .catch((err) => console.error("Error fetching units:", err));

    axios
      .get("backen-production-e1ec.up.railway.app/api/positions")
      .then((res) => setPositions(res.data))
      .catch((err) => console.error("Error fetching positions:", err));

    if (id !== "new") {
      axios
        .get(`backen-production-e1ec.up.railway.app/api/employees/${id}`)
        .then((res) => {
          const emp = res.data;
          setFormData({
            name: emp.name,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id === "new") {
        await axios.post(
          "backen-production-e1ec.up.railway.app/api/employees",
          dataToSend
        );
        Swal.fire("Success!", "Employee berhasil ditambahkan!", "success");
      } else {
        await axios.put(
          `backen-production-e1ec.up.railway.app/api/employees/${id}`,
          dataToSend
        );
