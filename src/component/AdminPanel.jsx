...
const AdminPanel = () => {
  ...
  const fetchUnits = async () => {
    const response = await axios.get(
      "backen-production-e1ec.up.railway.app/api/units"
    );
  ...
  const fetchPositions = async () => {
    const response = await axios.get(
      "backen-production-e1ec.up.railway.app/api/positions"
    );
  ...
  const addUnit = async () => {
    ...
    try {
      await axios.post("backen-production-e1ec.up.railway.app/api/units", {
        name: newUnit,
      });
      setNewUnit("");
      fetchUnits();
  ...
  const addPosition = async () => {
    ...
    try {
      await axios.post("backen-production-e1ec.up.railway.app/api/positions", {
        name: newPosition,
      });
      setNewPosition("");
      fetchPositions();
  ...
  const deleteItem = async (id, type) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `backen-production-e1ec.up.railway.app/api/${type}/${id}`
          );
